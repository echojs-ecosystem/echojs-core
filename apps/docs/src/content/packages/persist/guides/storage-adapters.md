...
description:
  localStorage, sessionStorage, cookies, IndexedDB, memory, and custom adapters.
package: '@echojs-ecosystem/persist'
---

# Storage Adapters

Choose an adapter extension based on **size**, **lifetime**, and **transport**
requirements.
...
Best for user preferences and medium-sized JSON snapshots. Supports `syncTabs`
...
> Cookie limits (~4KB) and cookies are sent on every request — use for tokens
> and small flags only.
...
If IndexedDB is unavailable, dev builds may fall back to in-memory storage with
a warning.
