...
`@echojs-ecosystem/i18n` provides typed messages, lazy locale loading, and
browser auto-detect. These defaults shape key inference and runtime behavior.
...
Keep **one eager locale** for `TranslationKey` inference. All-lazy locales make
keys loose.
...
Use `i18n.t()` inside **`effect` / `computed`** or HyperDOM reactive children —
reads `$locale` and re-runs when locale changes.
