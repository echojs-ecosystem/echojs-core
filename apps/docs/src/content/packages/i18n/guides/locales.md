...
Define supported locales in a `locales` map and switch at runtime with
`setLocale`.
...
Call **`i18n.start()`** (via provider `setup`) to apply detect, sync
`document.documentElement.lang`, optional `document.title`, and persist locale
changes.
