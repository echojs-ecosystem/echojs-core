...
description:
  docContentQuery with long staleTime and SEO effect in DocArticle model.
package: '@echojs-ecosystem/async'
---

# Docs Markdown Loader

The docs site loads markdown content per route using a query with a long
`staleTime` — content rarely changes during a session.

## Problem

Fetch and parse markdown when `contentId` changes, update document SEO from
frontmatter, and cache aggressively.
...
- **`staleTime: 3_600_000`** — avoid re-fetching on every navigation within the
  hour
