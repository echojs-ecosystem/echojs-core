...
The query demo model binds two query definitions to reactive params — a list
query with no params and a detail query keyed on a selected user ID signal.

## Problem

When the user picks a different ID from a list, refetch the detail query
automatically without manual `refetch()` calls.
...
- **`getUserQuery.with(() => ({ id: ... }))`** — re-runs when `$selectedUserId`
  changes
