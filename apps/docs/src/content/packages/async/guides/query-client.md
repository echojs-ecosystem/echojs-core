...
The **QueryClient** owns the in-memory cache. EchoJS apps typically share one
client via `createQueryProvider`; advanced cases use imperative APIs directly.
...
Access the client: `getQueryProvider()?.client` or pass `{ client }` to
`.with()`.

## Creating a client

```ts
import { createQueryClient } from '@echojs-ecosystem/async'
...
- [Installation](/docs/packages/async/installation)
- [API: QueryClient & Provider](/docs/packages/async/api/query-client)
- [Examples: Provider Defaults](/docs/packages/async/examples/provider-defaults)
