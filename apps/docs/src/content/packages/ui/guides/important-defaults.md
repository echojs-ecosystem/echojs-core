...
`@echojs-ecosystem/ui` is a HyperDOM component library — no React, no VDOM. These conventions shape how you import and compose controls.
...
Components are **functions** that return DOM nodes — call them inside views or pass as `children`:
...
Components support headless / `data-*` hooks — behavior and ARIA without visual Tailwind classes. Useful for custom design systems.
...
> [!note]
> Per-component a11y docs are **expanding**. See Storybook stories for current coverage.
