...
Form primitives compose through `Field` — label, control, error, and ARIA wiring in one place.
...
- **Controlled values** — bind `value` / `onInput` from signals inside reactive view children
- **Errors** — pass `error` string to `Field`; control receives `aria-invalid`
- **Masks** — input-mask helpers in `@echojs-ecosystem/ui/utils` (where exported)

> [!note]
> Per-control docs are **expanding**. See [Forms guide](/docs/guides/forms) for cross-package form patterns and `packages/ui` Storybook for variants.
