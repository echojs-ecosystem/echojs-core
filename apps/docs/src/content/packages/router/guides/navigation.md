...
Navigate imperatively via page objects or declaratively with HyperDOM link components.
...
Each page exposes `$isOpened`, `$params`, `$query`, `$data`, `$pending`, `$error`, `go()`, `open()`, and `preload()` (lazy pages).
