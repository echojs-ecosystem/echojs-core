...
Split page views into async chunks with `createLazyRouteView`. Chunks are cached
after first load.
...
Default export of the chunk must be a **`RouteView` function**. Use
`page.preload()` to warm the chunk before navigation.
...
export const appRoutes = createRoutes([
  {
    ...
    children: [{ path: 'settings', name: 'settings', routeView: settingsPage }],
  },
])
```
