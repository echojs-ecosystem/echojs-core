...
`@echojs-ecosystem/cli` will scaffold EchoJS apps and generate feature slices
aligned with the ecosystem's feature-first layout.

> [!warning] **CLI is on the roadmap.** Commands are not implemented. Use
> `apps/example` as the reference layout until `echojs create` ships.
...
- CI-friendly `doctor --json` for layout validation (pairs with
