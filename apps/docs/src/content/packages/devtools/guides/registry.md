...
Two core primitives: a **node registry** for inspectable state and a
...
When devtools is disabled, `registerDevtoolsNode` returns a no-op handle — safe
in production builds.

## Node types

`store` · `query` · `mutation` · `router` · `persist` · `url-state` ·
`ui-provider` · `signal` · `custom`
...
`getSnapshot` runs when a consumer reads state — keep it fast and side-effect
free.
...
`DevtoolsRegistry` and `DevtoolsTimeline` are exported for advanced tooling —
most apps use the facade functions.
