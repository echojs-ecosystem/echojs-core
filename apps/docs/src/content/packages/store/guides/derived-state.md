...
When state can be computed from other stores, use **`select`** or **`combine`** instead of duplicating fields or manually syncing values. Both return **readonly** stores that recompute when sources change.
...
Sources can be `Store` or `ReadonlyStore`. The result is **readonly** — mutate sources, not `fullName`.
...
Both `select` and `combine` accept the same `equals` option as `createStore`. Use a custom comparator when derived values are objects or arrays that may be referentially new but semantically equal.
