...
description:
  Named mutations via withActions — the preferred pattern for store updates.
package: '@echojs-ecosystem/store'
---

# Actions with withActions

`withActions` is the preferred pattern for named store mutations. Action
factories are typed, discoverable, and keep views from calling raw `set` /
`update`.
...
- The **outer function** runs once at extend time — use it to close over the
  store
...
Each key becomes a method on the extended store with inferred parameter types
