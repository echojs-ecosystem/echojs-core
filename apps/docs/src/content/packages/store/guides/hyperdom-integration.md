...
Stores complement HyperDOM models: **models** own local UI state; **stores** hold shared domain state that spans routes, features, or the whole app.
...
HyperDOM reactive children (`() => themeStore.value()`) track `$value` automatically — you rarely need a manual `effect()` in views for DOM updates.
...
Keep mutations in store actions (`withActions`), not scattered `set` calls from views.
...
Persist exposes `store.persist.pause()`, `resume()`, `clear()` — see [Session Stores](/docs/packages/store/examples/session-stores).
