---
title: combine
description: combine(sources, combiner, options?) — merge multiple stores into one readonly store.
...
```ts
function combine<Sources extends Record<string, Store<unknown> | ReadonlyStore<unknown>>, Result>(
  sources: Sources,
...
Creates a **readonly store** from multiple source stores. Recomputes when any source changes.
