...
`@echojs-ecosystem/architect` analyzes your `src/` tree against `architect.config.ts` and reports layer, public API, and cross-slice violations.

## What it checks

- **Layer direction** — pages may import entities, not the reverse
- **Public API** — consumers import `index.ts`, not deep paths
- **Cross-slice imports** — no sibling feature importing another feature's internals
...
The docs site uses it in CI via `bun run architect`. Config: `apps/docs/architect.config.ts`.
