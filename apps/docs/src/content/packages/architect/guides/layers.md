...
Left = higher layer. `pages` may import `entities`; `entities` may not import `pages`.
...
`Forbidden dependency "widgets" <= "entities"` — an entity imported a widget. Move shared code up or down the layer stack.
