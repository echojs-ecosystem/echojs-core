...
title: Counter
description:
  Basic writable signal with increment action and reactive HyperDOM view.
package: '@echojs-ecosystem/reactivity'
---

# Counter

The smallest useful pattern: one number in a signal, actions that mutate it, a
view that re-renders only the reactive parts.

## Problem

Track a numeric value and update the DOM when it changes — without re-mounting
...
export const CounterView = createView(
  (vm: CounterVM) =>
    section({ class: () => ({ 'is-positive': vm.$count.value() > 0 }) }, [
...
`() => vm.$count.value()` registers a dependency — only that text node updates
when the count changes.

## Takeaways

- Prefer `$count.update(fn)` over read-then-set when the next value depends on
  the previous one
