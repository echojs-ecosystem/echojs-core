...
Two input signals feed a **full name** computed, which feeds a **greeting** computed — no manual sync when fields change.

## Problem

Display derived text (full name, greeting, validation message) that always stays in sync with editable inputs.
...
export const createGreetingModel = createModel((): GreetingVM => {
  ...
  const $greeting = computed(() => {
    ...
    return name.length > 12 ? `Long name: ${name}` : `Hello, ${name || "guest"}!`;
  });
...
Both inputs and the greeting paragraph re-run independently — only affected nodes update.
