...
A todo list is a **signal of objects**. Every change produces a **new array**
(and new item objects when toggling) — never mutate `.value()` in place.

## Problem

Render a dynamic list, toggle item state, add rows, and derive a count of
completed items.
