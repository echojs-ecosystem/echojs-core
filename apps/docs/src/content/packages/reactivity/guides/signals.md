---
title: Signals
description: Writable reactive cells — read, write, subscribe, and naming conventions.
package: "@echojs-ecosystem/reactivity"
---

# Signals

A **signal** is a mutable reactive cell. Reads via `.value()` track dependents; writes via `.set()` / `.update()` notify subscribers when the value actually changes.
...
**`initial` is required** — calling `signal()` with no argument throws `TypeError`.
...
If the value is **purely derived** from other signals, use [Computed Values](/docs/packages/reactivity/guides/computed) — it caches and avoids manual sync.
