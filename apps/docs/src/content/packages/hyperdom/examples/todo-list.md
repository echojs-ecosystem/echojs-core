...
const todoList = () =>
  ul(null, List($items, (label, index) => li(null, () => `${index() + 1}. ${label}`)));
```

## With model VM

```ts
export const TodoView = createView((vm) =>
  ul(null, List(vm.$items, (item) => li(null, () => item.label))),
  "TodoView",
);
```
...
- Pair with [Reactivity: Todo List](/docs/packages/reactivity/examples/todo-list) for immutable array updates
