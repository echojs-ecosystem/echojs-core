...
The smallest useful HyperDOM app: one signal, one button, reactive text — no
model/view split.
...
- `() => \`Count: ${n.value()}\`` is a reactive child — only the text node
  updates
- `render()` returns `dispose` — call it when removing the view
- For larger apps, split into model + view — see
  [Model + View Counter](/docs/packages/hyperdom/examples/model-view-counter)
