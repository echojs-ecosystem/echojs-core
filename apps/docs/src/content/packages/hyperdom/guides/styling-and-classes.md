...
HyperDOM supports strings, objects, arrays, and reactive getters for `class` and
`style` props.
...
`cx` accepts strings, arrays, and `{ "token": boolean }` maps —
Tailwind-friendly.
...
Use `.innerHTML` only for trusted content — see
[Trusted HTML](/docs/packages/hyperdom/guides/trusted-html).
