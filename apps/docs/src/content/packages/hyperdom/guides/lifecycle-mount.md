...
The lifecycle **`mount`** hook runs **after** a node is inserted into the DOM.
Import it from the dedicated subpath — not the app `mount()` helper on the main
entry.
...
> [!WARNING] `mount` from `@echojs-ecosystem/hyperdom` is the **app mount
> helper** (`mount(view, options)`). The lifecycle hook lives at
...
The callback runs inside an active cleanup scope. Returned cleanup functions
integrate with HyperDOM's disposal chain.

## Strict context

In strict mode, lifecycle `mount()` must be called inside `createView`,
`render`, or an active scope — same rules as `h()`.
