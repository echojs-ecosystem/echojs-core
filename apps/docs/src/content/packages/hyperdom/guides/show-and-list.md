...
`Show` and `List` return **dynamic children** — function regions that re-evaluate when reactive dependencies inside their callbacks change.
...
Returns `() => Child` — use it as a child in a parent tree. Omit the third argument for no fallback (`null`).
