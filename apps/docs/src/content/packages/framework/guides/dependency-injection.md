...
`EchoApp` exposes a minimal DI surface — store values at bootstrap, read them in features.
...
When `provideKey` is set on `createProvider`, the instance from `install()` is automatically provided:
...
Router uses `ROUTER_KEY` from `@echojs-ecosystem/framework/app` — or the `injectRouter(app)` helper:
