...
title: createEchoApp
description:
  Options, static roots, provider-driven SPAs, and teardown with createEchoApp.
package: '@echojs-ecosystem/framework'
---

# createEchoApp

`createEchoApp` builds the composition root — register providers, resolve the
root view, and mount into the DOM.
...
When a router provider registers `resolveRoot()`, it overrides the static `view`
option.
...
`use()` is chainable. `provide()`, `inject()`, and `has()` are available on the
returned `EchoApp` instance.
...
- [createEchoApp API](/docs/packages/framework/api/create-echo-app) — method
  signatures
