...
`@echojs-ecosystem/framework` is a **meta package** — it re-exports ecosystem packages for convenience while keeping `/app` as the bootstrap entry.

## Bootstrap: always `/app`

```ts
import { createEchoApp, createProvider, ROUTER_KEY } from "@echojs-ecosystem/framework/app";
```
...
Size-sensitive apps import each package directly rather than pulling the full barrel.

## Related

- [Installation](/docs/packages/framework/installation) — typical stack
- [App Exports API](/docs/packages/framework/api/app-exports) — full barrel table
