...
description: .with() and withStore — bind query definitions to reactive params in models.
package: "@echojs-ecosystem/query"
---

# Reactive Binding

Query definitions are templates. **Instances** are created in models via `.with()` — they re-fetch when tracked signals change.
...
- [Examples: Query Demo Model](/docs/packages/query/examples/query-demo-model)
