...
description: createInfiniteQuery — paginated data, fetchNextPage, and flatMap helpers.
package: "@echojs-ecosystem/query"
---

# Infinite Queries

**Infinite queries** load data in pages — ideal for feeds, tables with "load more", and cursor-based APIs.
...
export const userPostsInfinite = createInfiniteQuery<PostsPage, { userId: number }, number>({
  ...
  initialPageParam: 0,
  queryFn: async ({ params, pageParam, signal }) => { /* fetch page */ },
...
`$data`, `$pages`, `$pageParams`, `$pending`, `$fetching`, and pagination-specific helpers like `hasInfiniteNextPage`.
