...
Views should never call `fetch` directly. Use query instance status helpers with HyperDOM `Show` for loading, error, and success states.

## Problem

Render loading spinner, error message, or data based on query status — without nested `if` chains that break reactivity.
...
For non-Show patterns, read `query.$status.value()` or `query.$fetchStatus.value()` inside reactive children.
