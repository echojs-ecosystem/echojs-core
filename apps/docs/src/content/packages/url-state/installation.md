...
`@echojs-ecosystem/url-state` syncs typed query parameters with reactive state — via the EchoJS router or standalone browser/memory adapters.
...
import { createQueryParams, parseAsString } from "@echojs-ecosystem/framework/url-state";
```
...
- [Important Defaults](/docs/packages/url-state/guides/important-defaults) — behavior overview
