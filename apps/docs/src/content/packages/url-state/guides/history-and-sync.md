---
title: History & Sync
description: push vs replace, defaultVisibility, debounce/throttle, and scroll options.
package: "@echojs-ecosystem/url-state"
---

# History & Sync

URL writes accept per-call and group-level options that control browser history, visibility of defaults, and update rate.
...
Typical convention: **`replace`** for filter typing, **`push`** for pagination steps.
...
Pass `equals` in set options when parsed objects need custom change detection beyond parser `eq`.
...
Prefer `.value()` in models; reactive children track URL state automatically when the adapter notifies.
