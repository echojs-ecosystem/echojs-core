...
description:
  Ephemeral signals in models — tabs, modals, and interaction state scoped to a
  feature.
...
**Local UI state** is **short-lived interaction state** that belongs to one
screen, widget, or feature: which tab is selected, whether a menu is open, hover
index, debounced search input before it hits the URL. It is implemented with
`signal` / `computed` / `effect` inside a **`createModel`** — not global stores.
...
Local state is where `effect()` belongs (focus trap, listen to escape key, sync
title):
...
- `shared/layout/mobile-nav.ts` — shell-level signal (shared layout, still not a
  product store)
