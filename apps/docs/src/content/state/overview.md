...
description: Types of state in EchoJS — router, form, server, URL, store, and local UI — and how they differ.
keywords: [state, router, form, query, url-state, store, signals]
---

# State overview

EchoJS does not use one global Redux-style store. Applications combine **several state layers**, each with a clear job. Mixing them (for example caching API data in route params) causes bugs and duplicate sources of truth.
...
- **Router** owns *which page* is open and *path* segments (`/users/42`).
- **URL state** owns *optional query keys* you want typed, validated, and easy to reset (often overlaps raw router query — prefer url-state for app filters).
- **Server state** is *not* in the URL unless you deliberately mirror an id there.
...
> [!RECOMMENDATION]
> Models (`createModel`) **orchestrate** — they read router/query/store/form and expose a small VM to the view. Views do not choose layers themselves.
...
Task-oriented walkthroughs (routing, fetching, forms, auth) live under [Guides](/docs/guides/routing). Package reference: [Router](/docs/packages/router/overview), [Query](/docs/packages/query/overview), [Store](/docs/packages/store/overview), [URL State](/docs/packages/url-state/overview).
