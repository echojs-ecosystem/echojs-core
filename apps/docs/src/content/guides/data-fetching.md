...
and route components stay declarative; models and API modules own the wiring.
...
Place **before** `routerProvider` if routes prefetch in `beforeLoad` using the
shared client.
...
Use stable **`queryKey` tuples** (`as const`). Optional `transform` maps the raw
response before cache storage.
...
Signals: `query.$data`, `query.$status`, `query.$fetchStatus` for
`effect`/`computed`.
...
Call `mutation.mutate(payload)` from model actions; read `isPending()` /
`error()` in the VM.
...
You can combine both: `beforeLoad` for access checks, Query for editable lists
on the same page.
...
- Surface `query.error()` in the VM; global error boundaries are optional route
  `errorView`s.
