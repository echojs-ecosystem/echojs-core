...
description: Session stores, persisted tokens, route guards, and login flows in EchoJS SPAs.
...
> [!DANGER]
> Never store refresh tokens or secrets in client-readable storage without threat modeling. Prefer **httpOnly** cookies set by your backend for primary session credentials.
...
Forms: [Forms guide](/docs/guides/forms). Example page: `apps/example/src/pages/auth/login/`.
...
Invalidate user queries on logout; optional `beforeLoad` redirect if session is missing.

## UI session block

Shell widgets can read `$authUser` / `$isLoggedIn` and call `authLoginPage.go()` or `logout()` — see `apps/example/src/widgets/docs-shell/docs-sidebar.ts`.

## SSR and OAuth

This guide targets **client SPAs** (`createEchoApp` + `history: "browser"`). SSR and OAuth redirects depend on your host (Node, edge, static). Keep callback URLs on the server; exchange code for tokens server-side when possible.
...
- Example — `apps/example/src/entities/session/`, `entities/__routes__/guards.ts`
