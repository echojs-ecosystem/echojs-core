...
Register with `createEchoApp().use(i18nProvider)` **before** routes that read translations in `beforeLoad` if needed.

`setup()` calls `i18n.start()` — applies detect rules, sets `document.documentElement.lang`, optional `document.title`, and persists locale changes to `storageKey`.
...
Do **not** cache translated strings in plain variables outside signals/effects unless the locale is fixed.
...
Build a locale picker with `i18n.supportedLocales` and labels from `i18n.t("locale.ru")` keys.
