...
description:
  Bootstrap an EchoJS SPA with createEchoApp, providers, and client-side
  routing.
---

# First Application

This walkthrough mirrors `apps/docs` and `apps/example`: one HTML shell, one
mount target, providers registered on `createEchoApp()`, and the router as the
root view.
...
import { queryProvider, routerProvider, uiProvider } from './providers/index.js'
...
- awaits async `setup()` hooks on providers (when `awaitProviders` is true,
  default)
- resolves the **root view** from the router provider (or a custom `view`
  option)
- calls HyperDOM `render()` into `#app`
- returns `dispose()` for teardown

> [!NOTE] Order matters when providers depend on each other — register
> query/theme before router if routes read them in `beforeLoad`.
...
`createRouterProvider` calls `router.start()` and exposes `router.View` as the
app root.
...
`createHomeModel` returns signals and actions; `HomeView` receives the VM and
returns HyperDOM nodes.
...
`NavLink` prevents default navigation and calls `aboutPage.go()` — **SPA
...
Open the dev server URL from Vite. You should see client-side transitions when
using `NavLink`.

## Try the lab

The `apps/example` project adds Hub → Docs Example / Workspace modules with
nested layouts and lazy routes:
