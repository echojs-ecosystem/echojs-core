...
title: Models
description:
  State and behavior with createModel — signals, effects, queries, and view
  models.
...
In EchoJS, a **model** is the place for state and behavior. Views read a **view
model (VM)** object and render HyperDOM; they do not own business logic.
...
Always pass a **display name** (second argument) — it appears in debug tooling
and error messages.
...
Home page model (`apps/docs`) keeps `$codeTab` private and exposes
`isCodeTabActive(index)` — views stay declarative and tests can target behavior.
...
HyperDOM `createModel` runs inside a cleanup scope; effects registered there
dispose when the model region unmounts.
...
const docContentQuery = createQuery({
  ...
  queryFn: async ({ params }) =>
    parseMarkdown(await loadContentRaw(params.contentId)),
})
...
Views use `query.isPending()`, `query.data()`, `query.error()` — no manual
loading flags.

## Props and model lifetime

`createComponent(model, view)()` invokes the model factory **once per call**.
For props that change per navigation, pass a **fresh** factory each time:
`createComponent(createDocArticleModel(props), view)()` so each `contentId` gets
a new model instance.

> [!NOTE] If props change but the same model instance is reused, stale state can
> leak between doc pages — always match lifetime to route params.
...
Do not import models from `pages/` into other pages — lift shared logic to
`features/` or `core/`.
