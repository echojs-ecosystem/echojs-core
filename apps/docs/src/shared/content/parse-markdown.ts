...
const parseFrontmatter = (raw: string): { meta: DocFrontmatter; body: string } => {
  if (!raw.startsWith("---\n")) {
  ...
  for (const line of header.split("\n")) {
    ...
    if (key === "keywords") meta.keywords = value.split(",").map((k) => k.trim());
  }
  return { meta, body };
};
...
const dropDuplicatePageTitle = (meta: DocFrontmatter, blocks: DocBlock[]): DocBlock[] => {
  const first = blocks[0];
  if (first?.type !== "heading" || first.level !== 1 || !meta.title) return blocks;
...
const parseCalloutLine = (line: string): { variant: ReturnType<typeof normalizeCalloutVariant>; title?: string } => {
  const m = line.match(/^>\s*\[!(\w+)\]\s*(.*)$/i);
  if (!m) return { variant: "note" };
  return { variant: normalizeCalloutVariant(m[1]!), title: m[2]?.trim() || undefined };
};
...
export const parseMarkdown = (raw: string): DocDocument => {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("> [!")) {
      ...
      blocks.push({ type: "callout", variant, title, body: bodyLines.join("\n").trim() });
      continue;
    }
