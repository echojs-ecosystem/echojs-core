...
import { button, div, h, input, label, option, p, select, Show, span, textarea } from "@echojs-ecosystem/framework/hyperdom";
    ...
    div({ class: form.form() }, [
        ...
        select(
          {
            ...
            ...bindField(category, { variant: "select", controlledValue: true }),
          },
          ideaCategoryOptions.map((opt) => option({ value: opt.value }, opt.label)),
        ),
      ...
      label({ class: [form.field(), form.fieldFull()].join(" ") }, [
        ...
        textarea({
          ...
          ...bindField(description, { variant: "textarea", controlledValue: true }),
        }),
      ...
      p({ class: form.hint() }, "Powered by createForm — validation runs on submit."),
      Show(
        submitSuccess,
        () => {
