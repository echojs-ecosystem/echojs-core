...
import { ideaCategoryOptions, roadmapColumns, roadmapGithubRepo } from "@entities/roadmap/constants/roadmap.data.js";
import type {
  CommunityIdea,
  IdeaCategory,
  IdeaFormFields,
...
const buildGithubIssueUrl = (idea: IdeaFormValue): string => {
  const categoryLabel = ideaCategoryOptions.find((opt) => opt.value === idea.category)?.label ?? idea.category;
  ...
  const body = [
    ...
    idea.github.trim() ? `**GitHub:** @${idea.github.trim().replace(/^@/, "")}` : "",
    "",
...
export const createRoadmapModel = createModel((): RoadmapVM => {
  ...
  const ideaForm = createForm<IdeaFormValue, IdeaFormFields>(
    ...
    },
    {
      ...
      defaultValues: { title: "", description: "", category: "feature", github: "" },
