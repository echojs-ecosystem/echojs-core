...
export const sponsorsTierStyles = tv({
  slots: {
    panel:
      'overflow-hidden rounded-2xl border border-border/80 bg-surface-elevated/60 dark:border-white/10 dark:bg-surface-elevated/40',
    panelGold:
      'relative border-echo-500/25 before:absolute before:inset-y-3 before:left-0 before:w-0.5 before:rounded-full before:bg-gradient-to-b before:from-echo-400 before:to-echo-600',
    ...
    header:
      'flex items-center gap-2 border-b border-border/80 px-5 py-3.5 dark:border-white/10',
    ...
    headerLabelGold:
      'text-[11px] font-bold uppercase tracking-[0.18em] text-echo-600 dark:text-echo-400',
    headerLabelSilver:
      'text-[11px] font-bold uppercase tracking-[0.18em] text-fg-subtle',
    headerLabelBronze:
    ...
    cellName:
      'text-xs font-semibold text-fg-muted opacity-0 transition group-hover:opacity-100 sm:text-sm',
    cellNameVisible: 'text-xs font-semibold text-fg sm:text-sm',
    placeholderIcon:
      'flex h-14 w-14 items-center justify-center rounded-full border-2 border-dashed border-border text-xl text-fg-subtle dark:border-white/15',
