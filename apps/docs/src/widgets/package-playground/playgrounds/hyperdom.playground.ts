...
import { Show, button, div, p } from "@echojs-ecosystem/hyperdom";
...
const create = (): PlaygroundInstance => {
  const $n = signal(0);
  const $snapshot = signal<Record<string, unknown>>({ n: 0, showPositive: false });
  ...
  return {
    ...
    view: () => {
      const n = $n.value();
      return div(null, [
        p({ class: pg.hint() }, "Reactive children and Show — no full page reload."),
        div(
          {
            class: () =>
              [
          ...
          [
            ...
            Show(
              () => n > 0,
              () => p({ class: "mt-2 text-sm text-emerald-800 dark:text-emerald-200" }, "Positive branch"),
            ),
          ],
        ),
        div({ class: pg.actions() }, [
          button({ class: pg.btnPrimary(), onClick: () => $n.update((x) => x + 1) }, "Increment"),
