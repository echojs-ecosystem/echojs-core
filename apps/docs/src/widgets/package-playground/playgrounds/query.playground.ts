...
const create = (): PlaygroundInstance => {
  ...
  return {
    ...
    view: () =>
      div(null, [
        p({ class: pg.hint() }, "Uses the app QueryClient — ~600ms fake fetch per id."),
        p({ class: pg.row() }, () => `status: ${user.status()} · fetching: ${user.isFetching()}`),
        p({ class: pg.metric() }, () => user.data()?.name ?? "…"),
        div({ class: pg.actions() }, [
          button({ class: pg.btn(), onClick: () => $userId.update((id) => Math.max(1, id - 1)) }, "Id −"),
          button({ class: pg.btnPrimary(), onClick: () => $userId.update((id) => id + 1) }, "Id +"),
          button({ class: pg.btn(), onClick: () => void user.refetch() }, "Refetch"),
        ]),
      ]),
  };
};

export const queryPlayground: PackagePlaygroundDef = {
  id: "query",
