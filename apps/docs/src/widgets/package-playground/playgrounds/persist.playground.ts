  ...
  return {
    ...
    view: () =>
      div(null, [
        p(
          { class: pg.hint() },
          'Memory storage — survives clicks until you clear.'
        ),
        ...
        div({ class: pg.actions() }, [
          button(
            {
              class: pg.btnPrimary(),
              onClick: () => counter.update((v) => v + 1),
            },
            '+1'
          ),
          button(
            { class: pg.btn(), onClick: () => void counter.persist.save() },
            'Save'
          ),
          button(
            { class: pg.btn(), onClick: () => void counter.persist.clear() },
            'Clear storage'
          ),
        ]),
      ]),
