...
import { button, div, p, type Child } from "@echojs/hyperdom";
...
const create = (): PlaygroundInstance => {
  ...
  effect(() => {
    $snapshot.set({
      ...
      activePage: (router.$activePage.value() as { name?: string } | null)?.name ?? null,
  ...
  return {
    ...
    view: () =>
      div(null, [
        p({ class: pg.hint() }, "Memory history — outlet below, router signals in state."),
        div(
          {
            class:
        ...
        div({ class: pg.actions() }, [
          button({ class: pg.btnPrimary(), onClick: () => router.go("/") }, "Go home"),
          button({ class: pg.btn(), onClick: () => router.go("/about") }, "Go about"),
