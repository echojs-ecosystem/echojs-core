...
import { workspaceBootstrap } from '../model/settings.model'
...
export const SettingsView = (): Child =>
  div({ class: layout.page() }, [
    ...
    section({ class: `${layout.card()} mt-4` }, [
      ...
      pre({ class: 'overflow-auto text-xs text-fg-muted' }, () =>
        JSON.stringify(workspaceBootstrap.data() ?? { status: workspaceBootstrap.status() }, null, 2),
      ),
    ]),
    section({ class: `${layout.card()} mt-4` }, [
