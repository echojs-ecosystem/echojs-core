...
const formatLocation = (
  location: AugmentedDiagnostic["location"],
  cwd: string,
) => {
...
export const formatSingleDiagnostic = (
  d: AugmentedDiagnostic,
  cwd: string,
): string => {
  const x =
    d.rule.severity === "error"
  ...
  const autofixable =
    d.fixes !== undefined && d.fixes.length > 0
      ? chalk.green(`${figures.tick} Auto-fixable`)
      : null;
  const location = chalk.gray(formatLocation(d.location, cwd));
  const ruleName = d.rule.descriptionUrl
    ? chalk.blue(terminalLink(d.rule.name, d.rule.descriptionUrl!))
    : d.rule.name;
