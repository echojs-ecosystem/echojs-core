 ...
 */
export const mountLifecycleHook = (fn) => {
    if (getStrictContextChecks() && !isInViewContext() && !getCurrentScope()) {
        throw new Error("hyperdom: mount() called outside of view/render context. Call it while rendering (as a child), or inside createView(...).");
    }
