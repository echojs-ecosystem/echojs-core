...
import { parseQuery, parseQueryValues, stringifyQuery } from "./query";
...
describe("query", () => {
  ...
  it("parseQueryValues keeps repeated keys as arrays", () => {
    expect(parseQueryValues("?tag=a&tag=b")).toEqual({ tag: ["a", "b"] });
  });

  it("parseQueryValues coerces boolean literals", () => {
    expect(parseQueryValues("?enabled=true&disabled=false")).toEqual({
      enabled: "true",
      disabled: "false",
    });
  });
