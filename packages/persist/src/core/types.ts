...
export type PersistOptions<Value, Snapshot = Value> = {
  ...
  onHydrate?: (ctx: { value: Value; snapshot: Snapshot }) => void;
  onSave?: (ctx: { value: Value; snapshot: Snapshot }) => void;
...
export type PersistExtension<Value, Snapshot = Value> = (
  target: Persistable<Value>,
) => PersistExtensionResult<Value, Snapshot>;

export type CookiePersistOptions<Value, Snapshot = Value> = PersistOptions<
  Value,
  Snapshot
> & {
  path?: string;
...
export type IndexedDBPersistOptions<Value, Snapshot = Value> = PersistOptions<
  Value,
  Snapshot
> & {
  dbName?: string;
