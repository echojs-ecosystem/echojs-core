...
describe("createQueryParams", () => {
  ...
  it("update works", async () => {
    ...
    const filters = createQueryParams({ page: parseAsInteger.withDefault(1) }, { adapter });
  ...
  it("array values (repeated keys) in group", () => {
    ...
    const filters = createQueryParams({ tag: parseAsArrayOf(parseAsString).withDefault([]) }, { adapter });
