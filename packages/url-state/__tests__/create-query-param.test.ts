...
import { createMemoryUrlStateAdapter, createQueryParam, parseAsInteger, parseAsString } from "@echojs/url-state";
...
describe("createQueryParam", () => {
  ...
  it("multiple instances same key sync", async () => {
    const adapter = createMemoryUrlStateAdapter("?page=1");
    const a = createQueryParam("page", parseAsInteger.withDefault(1), { adapter });
    const b = createQueryParam("page", parseAsInteger.withDefault(1), { adapter });
    a.set(2);
    await flush();
