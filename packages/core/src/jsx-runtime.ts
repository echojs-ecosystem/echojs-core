...
import { isSignalish, isFunction, isString, isNullOrUndefined, isArray } from "./internals/utils";
import { createElement, setProp } from "./element";
import { setEvents, setEvent } from "./events";
import { insert } from "./insert";
import { createReactiveText } from "./text";
import { applyShowDirective } from "./directives/show";
import { _$if } from "./directives/if";
import type { JSXElement, JSXProps, JSXComponent, Signalish } from "./types";

// Re-export JSX types for TypeScript
  ...
  if (hasIfDirective) {
    ...
    const createThenBranch = (): JSXElement => {
      ...
      for (const [name, value] of Object.entries(props)) {
        ...
        // Events are attached once via setEvents(element, props) below
        if (name.startsWith("on:") || name.startsWith("on-") || /^on[A-Z]/.test(name)) {
          continue;
        }
